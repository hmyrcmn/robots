...
class MotorController:
    ...
    def _setup_state_transitions(self):
        ...
        # Her durum için gerekli koşullar
        self.state_conditions = {
            MotorState.CHECKING_VOLTAGE: lambda: self.voltage is not None,
            MotorState.CHECKING_STATUS: lambda: self.axis_state is not None,
            MotorState.CALIBRATING: lambda: self.axis_state == ODriveAxisState.AXIS_STATE_IDLE,
            MotorState.CALIBRATION_WAITING: lambda: self.procedure_result in [ProcedureResult.BUSY, ProcedureResult.SUCCESS],
            MotorState.TRANSITIONING_TO_CLOSED_LOOP: lambda: self.calibration_flag,
            MotorState.CLOSED_LOOP: lambda: self.axis_state == ODriveAxisState.AXIS_STATE_CLOSED_LOOP_CONTROL,
            MotorState.VELOCITY_CONTROL: lambda: self.axis_state == ODriveAxisState.AXIS_STATE_CLOSED_LOOP_CONTROL
        }

    def validate_state_transition(self, new_state: MotorState) -> bool:
        """
        ...
        Returns:
            bool: Geçiş geçerli ise True
        """
        # Geçerli geçiş kontrolü
        if new_state not in self.valid_transitions[self.current_state]:
            logging.error(f"Invalid state transition: {self.current_state.value} -> {new_state.value}")
            return False
            
        # Durum koşulları kontrolü
        if new_state in self.state_conditions:
            if not self.state_conditions[new_state]():
                logging.error(f"State conditions not met for transition to {new_state.value}")
                return False
    ...
    def handle_error(self, error: Exception = None) -> None:
        """
        Hata durumunu yönetir ve kurtarma mekanizmasını başlatır
        ...
        self.errors.append({
            ...
            'axis_state': self.axis_state.value if self.axis_state else None
        })
        
        # Hata sayısını kontrol et
        if len(self.errors) > 5:
            logging.critical("Too many errors, performing emergency shutdown")
        ...
        if self.retry_count < self.max_retries:
            ...
            # Hata durumuna göre kurtarma stratejisi belirle
            if self.current_state == MotorState.VELOCITY_CONTROL:
                self.recover_velocity_control()
            elif self.current_state == MotorState.CALIBRATING:
                self.recover_calibration()
            else:
                self.clear_error()
    ...
    def emergency_shutdown(self) -> None:
        """Acil durum kapatma işlemi"""
        try:
            logging.warning("Performing emergency shutdown...")
            # Motoru durdur
            ...
            logging.critical(f"Emergency shutdown failed: {e}")

    def recover_velocity_control(self) -> None:
        """Hız kontrolü kurtarma işlemi"""
        try:
            # Motoru durdur
            set_command(self.ser, "axis0.requested_state", ODriveAxisState.AXIS_STATE_IDLE.value)
            time.sleep(0.5)
            ...
            logging.error(f"Velocity control recovery failed: {e}")
            self.emergency_shutdown()

    def recover_calibration(self) -> None:
        """Kalibrasyon kurtarma işlemi"""
        try:
            # Motoru durdur
            set_command(self.ser, "axis0.requested_state", ODriveAxisState.AXIS_STATE_IDLE.value)
            time.sleep(0.5)
            # Hataları temizle
            self.clear_error()
            # Kalibrasyonu yeniden başlat
            self.change_state(MotorState.CALIBRATING)
        except Exception as e:
            logging.error(f"Calibration recovery failed: {e}")
            self.emergency_shutdown()
    ...
    def handle_initializing_state(self):
        ...
        try:
            ...
            if self.voltage is not None and self.voltage >= 10:
                # Voltaj uygunsa durum kontrolüne geç
                if self.validate_state_transition(MotorState.CHECKING_STATUS):

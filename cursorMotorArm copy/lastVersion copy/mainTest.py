...
class MotorController:
    ...
    def handle_calibrating_state(self):
        ...
        if success :
            ...
            time.sleep(1)
    def check_closed_loop_flag(self,v):
        """Kapalı döngü bayrağı kontrolü"""
        print("heyyyyyyyy:v ",v)
    ...
    def handle_calibration_waiting_state(self):
        ...
        if self.procedure_result == ProcedureResult.BUSY:
            ...
            #TODO : kontrolet sonra motoru kapalı döngüye geçirmek için
            run_command(self.ser,"axis0.config.startup_closed_loop_control",lambda v: self.self.check_closed_loop_flag(v))
            #self.change_state(MotorState.TRANSITIONING_TO_CLOSED_LOOP)
        else:
...
def main():
    ...
    try:
        ...
        if ser:
            ...
            data_collector = DataCollector(file_name="training_data.csv")

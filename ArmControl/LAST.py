...
class MotorController:
    ...
    def __init__(self, ser):
        ...
        self.state_entry_actions = {
            MotorState.INITIALIZING: self._on_enter_initializing,
            MotorState.ERROR: self._on_enter_error
        }
        self.state_exit_actions = {
            MotorState.ERROR: self._on_exit_error
        }
    ...
    def _validate_state_transition(self, new_state: MotorState) -> bool:
    ...
    def _on_enter_initializing(self):
        """Actions to perform when entering initializing state"""
        self.retry_count = 0
        self.clear_error()

    def _on_enter_error(self):
        """Actions to perform when entering error state"""
        logging.error(f"Entering error state from {self.current_state.value}")
        self.errors = []

    def _on_exit_error(self):
        """Actions to perform when exiting error state"""
        logging.info("Exiting error state")
        self.clear_error()
    ...
    def change_state(self, new_state: MotorState) -> None:
        ...
        if not self._validate_state_transition(new_state):
            logging.error(f"Invalid state transition attempted: {self.current_state.value} -> {new_state.value}")
            return

        # Execute exit actions for current state
        if self.current_state in self.state_exit_actions:
        ...
        if new_state in self.state_entry_actions:
            self.state_entry_actions[new_state]()
    ...
    def handle_velocity_control_state(self):
        ...
        try:
            ...
            if not run_command(self.ser, "axis0.current_state", lambda v: self.check_status(v)):
                logging.error("Failed to check motor state")
                self.change_state(MotorState.ERROR)
                return
            ...
            # Tork ölçümü ve temas kontrolü
            if not run_command(self.ser, "axis0.motor.foc.Iq_measured", lambda v: self.check_torque(v)):
                logging.error("Failed to read torque value")
                # Don't transition to error state immediately, just log the error
                return

            # Check if we need to handle direction change
            if abs(self.current_torque) > self.torque_threshold:
                current_time = time.time()
                if current_time - self.last_direction_change >= self.direction_change_cooldown:
                    # Reverse velocity
                    self.current_velocity = -self.current_velocity
                    if not set_command(self.ser, "axis0.controller.input_vel", self.current_velocity):
